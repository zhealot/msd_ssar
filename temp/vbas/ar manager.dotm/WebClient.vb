...
' MSD
' Contains a custom MSD Extension for dealing with authentication handshake with F5
...
Private Const web_DefaultTimeoutMs As Long = 17000
...
#Else
    ...
    'Custom MSD Extension
    If web_Response.Content = "<h1>SendInfopathToRemedy</h1>" & vbNewLine & "<p>Hi there, this is an AXIS service!</p>" & vbNewLine & "<i>Perhaps there will be a form for invoking the service here...</i>" & vbNewLine Then

        web_Http.Open WebHelpers.MethodToName(request.Method), Me.GetFullUrl(request), False

        web_Http.SetRequestHeader "SOAPAction", "InfopathWebServiceService"
...
''
Public Function GetJson(Url As String, Optional Options As Scripting.Dictionary) As WebResponse
...
' @return {WebResponse} Response
''
Public Function PostJson(Url As String, Body As Variant, Optional Options As Scripting.Dictionary) As WebResponse
    Dim web_Request As New WebRequest
    web_Request.CreateFromOptions Options
    web_Request.Resource = Url
    web_Request.Format = WebFormat.Json
...
#If Win32 Or Win64 Then
    ...
    Dim web_Parts As Scripting.Dictionary
